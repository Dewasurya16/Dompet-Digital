...
import {
  ...
  ArrowUpRight, ArrowDownRight, Tag, Settings, X, Moon, Sun, Filter, ArrowLeft,
...
const SwalToast = Swal.mixin({
  ...
  customClass: { popup: 'rounded-2xl shadow-xl text-sm font-semibold' },
  didOpen: (toast) => {
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const alreadySentToday = localStorage.getItem(lastBillReminderKey);
        ...
        if (daysLeft === 3 || daysLeft === 1) {
          fetch('/api/send-notification', {
            ...
            body: JSON.stringify({
              ...
              type: 'bill_reminder',
              data: { billName: bill.name, amount: bill.amount, daysLeft }
            })
          }).catch(console.error);
        }
      });
  ...
  useEffect(() => {
    if (!isMounted || !session?.user?.email || transactions.length === 0) return;
    const today = new Date();
    if (today.getDate() > 3) return; // Only run on first 3 days of month
    const lastMonth = today.getMonth() === 0 ? 11 : today.getMonth() - 1;
    const lastMonthYear = today.getMonth() === 0 ? today.getFullYear() - 1 : today.getFullYear();
    const recapKey = `monthly_recap_${lastMonthYear}_${lastMonth}`;
    ...
    const totalIncome = lastMonthTx.filter((t: any) => t.type === 'pemasukan').reduce((s: number, t: any) => s + t.amount, 0);
    const totalExpense = lastMonthTx.filter((t: any) => t.type === 'pengeluaran').reduce((s: number, t: any) => s + t.amount, 0);
    ...
    fetch('/api/send-notification', {
      ...
      body: JSON.stringify({
        email: session.user.email,
        type: 'monthly_recap',
        data: { monthName: monthNames[lastMonth], totalIncome, totalExpense }
      })
    }).catch(console.error);
    localStorage.setItem(recapKey, 'true');
  }, [transactions, isMounted, session]);
  ...
  const handleLogout = async () => {
    const result = await Swal.fire({
      ...
      text: 'Sesi kamu akan diakhiri. Sampai jumpa! 👋',
    ...
    if (editingId) {
      ...
      if (!error && data) {
        ...
        if (session?.user?.email) {
          ...
          // ── Anomaly Alert (pengeluaran > 10 juta) ──
          if (payload.type === 'pengeluaran' && payload.amount > 10000000) {
            fetch('/api/send-notification', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                email: session.user.email,
                type: 'anomaly_alert',
                data: { amount: payload.amount, category: payload.category, description: payload.title }
              })
            }).catch(console.error);
          }
  ...
  const handleSavePocket = async () => {
    ...
    let error;
    if (editingPocketId) {
      const { error: updateError } = await supabase.from('pockets').update(payload).eq('id', editingPocketId);
      error = updateError;
    } else {
      const { error: insertError } = await supabase.from('pockets').insert([payload]);
      error = insertError;
    }
    ...
    if (!error) {
      ...
      fetchData();
      showToast(editingPocketId ? 'Kantong tabungan diperbarui! 🎯' : 'Kantong tabungan berhasil dibuat! 🎯', 'success');

      // ── Goal Reached Check ──
      const currentBalance = Number(payload.balance);
      const targetAmt = Number(payload.target_amount);
      if (currentBalance >= targetAmt && targetAmt > 0 && session?.user?.email) {
        fetch('/api/send-notification', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            email: session.user.email,
            type: 'goal_reached',
            data: { pocketName: payload.name, targetAmount: targetAmt }
          })
        }).catch(console.error);
        showToast(`🏆 Target "${payload.name}" TERCAPAI! Cek email kamu!`, 'success');
      }
  ...
  const handleEditPocket = (pocket: any) => {
    ...
    setPocketForm({
      ...
      balance: pocket.balance.toString()
    });
    setShowPocketModal(true);
  };

  const handleDeletePocket = async (id: string) => {
    const ok = await confirm('Hapus Kantong Tabungan', 'Apakah Anda yakin ingin menghapus kantong ini? Semua data terkait akan hilang.', true);
    if (!ok) return;
    const { error } = await supabase.from('pockets').delete().eq('id', id);
    if (!error) {
      fetchData();
      showToast('Kantong tabungan berhasil dihapus.', 'info');
    } else {
      showToast('Gagal menghapus kantong: ' + error.message, 'error');
    }
  };
  ...
  const handleSaveInvest = async () => {
    ...
    let error;
    if (editingInvestId) {
      const { error: updateError } = await supabase.from('investments').update(payload).eq('id', editingInvestId);
      error = updateError;
    } else {
      const { error: insertError } = await supabase.from('investments').insert([payload]);
      error = insertError;
    }
    ...
    if (!error) {
      ...
      fetchData();
      showToast(editingInvestId ? 'Aset investasi diperbarui! 📈' : 'Aset investasi berhasil ditambahkan! 📈', 'success');
    } else showToast('Gagal menyimpan aset: ' + error.message, 'error');
  };

  const handleEditInvest = (invest: any) => {
    setEditingInvestId(invest.id);
    setInvestForm({
      name: invest.asset_name,
      asset_type: invest.asset_type,
      platform: invest.platform || '',
      invested_amount: invest.invested_amount.toString(),
      current_value: invest.current_value.toString()
    });
    setShowInvestModal(true);
  };

  const handleDeleteInvest = async (id: string) => {
    const ok = await confirm('Hapus Aset Investasi', 'Apakah Anda yakin ingin menghapus aset investasi ini?', true);
    if (!ok) return;
    const { error } = await supabase.from('investments').delete().eq('id', id);
    if (!error) {
      fetchData();
      showToast('Aset investasi berhasil dihapus.', 'info');
    } else {
  ...
  const handleSaveBill = async () => {
    if (!billForm.name || !billForm.amount || !billForm.due_date) return showToast('Nama, nominal, dan tanggal jatuh tempo wajib diisi!', 'warning');
    const dueDay = Number(billForm.due_date);
    if (dueDay < 1 || dueDay > 31) return showToast('Tanggal jatuh tempo harus antara 1-31!', 'warning');
    ...
    let error;
    if (editingBillId) {
      const { error: updateError } = await supabase.from('bills').update(payload).eq('id', editingBillId);
      error = updateError;
    } else {
    ...
    if (!error) {
      ...
      setBillForm({ name: '', amount: '', due_date: '', category: 'Tagihan' });
      fetchData();
      showToast(editingBillId ? 'Tagihan diperbarui! 📅' : 'Tagihan berhasil ditambahkan! 📅', 'success');
    } else showToast('Gagal menyimpan tagihan: ' + error.message, 'error');
  };

  const handleEditBill = (bill: any) => {
    setEditingBillId(bill.id);
    setBillForm({ name: bill.name, amount: bill.amount.toString(), due_date: bill.due_date.toString(), category: bill.category });
    setShowBillModal(true);
  };

  const handleDeleteBill = async (id: string) => {
    const ok = await confirm('Hapus Tagihan', 'Tagihan ini akan dihapus secara permanen.', true);
    if (!ok) return;
    const { error } = await supabase.from('bills').delete().eq('id', id);
    if (!error) { fetchData(); showToast('Tagihan berhasil dihapus.', 'info'); }
    else showToast('Gagal menghapus tagihan: ' + error.message, 'error');
  };
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] dark:bg-[#09090B] text-slate-900 dark:text-slate-100 font-sans antialiased transition-colors duration-500">
      ...
      {/* ═══ DESKTOP SIDEBAR ═══ */}
      <aside className="hidden lg:flex flex-col fixed left-0 top-0 bottom-0 w-[250px] bg-white dark:bg-[#111] border-r border-slate-200/60 dark:border-slate-800/60 z-50 p-5 overflow-y-auto">
        {/* Logo */}
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-4 mb-6 border border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full flex items-center justify-center text-white font-black text-base shadow-md shrink-0">
              {displayUsername.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-sm text-slate-800 dark:text-white truncate">{displayUsername}</p>
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-3">
            ...
            {gamification.streak >= 3 && <div className="text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 bg-amber-100 text-amber-600 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800"><Zap size={10} /> {gamification.streak}d</div>}
            {gamification.isIronSaver && <div className="text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800"><ShieldAlert size={10} className="text-emerald-500" /> Iron Saver</div>}
          </div>
          {/* Logout Button */}
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/40 border border-rose-100 dark:border-rose-900/50 transition-all duration-200 font-bold text-xs group"
          >
      ...
      <div className="lg:ml-[250px] min-h-screen pb-28 lg:pb-0">
        ...
        <header className="hidden lg:flex items-center justify-between px-8 py-5 border-b border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-[#111]/50 backdrop-blur-sm sticky top-0 z-40">
          <h1 className="text-2xl font-black tracking-tight text-slate-800 dark:text-white">
            {activeView === 'dashboard' ? 'Overview' : activeView === 'transactions' ? 'Transaksi' : activeView === 'analytics' ? 'Analitik' : activeView === 'wallets' ? 'Dompet' : activeView === 'bills' ? 'Tagihan & Pengingat' : 'Pengaturan'}
          </h1>
          <div className="flex items-center gap-3">
            {/* Filters */}
        ...
        <div className="p-4 sm:p-6 lg:p-8">
                      ...
                      <div key={p.id} className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-gradient-to-br from-indigo-50 to-slate-50 dark:from-indigo-900/10 dark:to-slate-900">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="font-black text-sm text-slate-800 dark:text-white leading-tight">{p.name}</p>
                              <p className="text-[10px] text-slate-400">{pct}% tercapai</p>
                            </div>
                          </div>
                          <div className="flex gap-1">
                            <button onClick={() => handleEditPocket(p)} className="p-1.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors"><Edit2 size={14} /></button>
                            <button onClick={() => handleDeletePocket(p.id)} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg transition-colors"><Trash2 size={14} /></button>
                          </div>
                        </div>
                      ...
                      <div key={inv.id} className={`p-4 rounded-xl border bg-gradient-to-br ${isUp ? 'from-teal-50 to-slate-50 border-teal-100 dark:from-teal-900/10 dark:to-slate-900 dark:border-teal-900' : 'from-rose-50 to-slate-50 border-rose-100 dark:from-rose-900/10 dark:to-slate-900 dark:border-rose-900'}`}>
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex-1 pr-2">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{inv.asset_type}</p>
                            <p className="font-black text-sm text-slate-800 dark:text-white mt-0.5">{inv.asset_name}</p>
                            {inv.platform && <p className="text-[10px] text-slate-400">{inv.platform}</p>}
                          </div>
                          <div className="flex flex-col items-end gap-1">
                            <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${isUp ? 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400'}`}>{isUp ? '+' : ''}{pct.toFixed(1)}%</span>
                            <div className="flex gap-1 mt-1">
                              <button onClick={() => handleEditInvest(inv)} className="p-1 text-slate-400 hover:text-blue-500 transition-colors"><Edit2 size={12} /></button>
                              <button onClick={() => handleDeleteInvest(inv.id)} className="p-1 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={12} /></button>
                            </div>
                          </div>
                        </div>
          ...
          {activeView === 'bills' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white">Tagihan & Pengingat</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Email reminder otomatis dikirim H-3 dan H-1 sebelum jatuh tempo</p>
                </div>
                <button
                  onClick={() => { setEditingBillId(null); setBillForm({ name: '', amount: '', due_date: '', category: 'Tagihan' }); setShowBillModal(true); }}
                  className="flex items-center gap-2 bg-blue-600 text-white font-bold px-4 py-2.5 rounded-xl hover:bg-blue-700 active:scale-[0.98] transition-all shadow-lg shadow-blue-500/25 text-sm"
                >
              ...
              {bills.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 px-6 bg-white dark:bg-[#111] rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 text-center">
                  ...
                  <h3 className="font-black text-slate-700 dark:text-slate-300 text-lg mb-2">Belum Ada Tagihan</h3>
                  <p className="text-slate-400 text-sm mb-6 max-w-xs leading-relaxed">
                    Tambahkan tagihan rutin seperti Netflix, Spotify, atau listrik agar tidak pernah terlewat.
                  </p>
                  <button
                    onClick={() => { setEditingBillId(null); setBillForm({ name: '', amount: '', due_date: '', category: 'Tagihan' }); setShowBillModal(true); }}
                    className="flex items-center gap-2 bg-blue-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-blue-700 active:scale-[0.98] transition-all shadow-lg shadow-blue-500/25 text-sm"
                  >
                    <Plus size={16} /> Tambah Tagihan Pertama
                  </button>
                </div>
              ) : (
                <div className="space-y-3">
                  {bills.map((bill: any) => {
                    ...
                    return (
                      <div key={bill.id} className={`bg-white dark:bg-[#111] rounded-2xl p-5 border transition-all hover:shadow-md ${isOverdue ? 'border-rose-200 dark:border-rose-900 bg-rose-50/30 dark:bg-rose-950/10' : isUrgent ? 'border-amber-200 dark:border-amber-900 bg-amber-50/30 dark:bg-amber-950/10' : 'border-slate-200/60 dark:border-slate-800'} flex items-center gap-4`}>
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${isOverdue ? 'bg-rose-100 dark:bg-rose-900/30' : isUrgent ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-blue-100 dark:bg-blue-900/30'}`}>
                          <Bell size={20} className={isOverdue ? 'text-rose-600' : isUrgent ? 'text-amber-600' : 'text-blue-600'} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="font-black text-slate-900 dark:text-white truncate">{bill.name}</p>
                            {isOverdue && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400 border border-rose-200 dark:border-rose-800 shrink-0">LEWAT</span>}
                            {isUrgent && !isOverdue && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800 shrink-0">SEGERA</span>}
                          </div>
                          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            {formatIDR(bill.amount)} · Tgl {bill.due_date} setiap bulan
                            {daysLeft >= 0 && <span className={`ml-2 font-bold ${isUrgent ? 'text-amber-600 dark:text-amber-400' : 'text-slate-600 dark:text-slate-300'}`}>({daysLeft === 0 ? 'Hari ini!' : `${daysLeft} hari lagi`})</span>}
                          </p>
                        </div>
                        <div className="flex gap-2 shrink-0">
                          <button
                            onClick={() => handleEditBill(bill)}
                            className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all"
                            title="Edit"
                          >
                            <Edit2 size={15} />
                          </button>
                          <button
                            ...
                            className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-all"
                            title="Hapus"
                          >
                            <Trash2 size={15} />
                          </button>
      ...
      {/* ═══ BILL MODAL ═══ */}
      {showBillModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-end sm:items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) { setShowBillModal(false); setEditingBillId(null); } }}>
          <div className="bg-white dark:bg-[#1A1A1A] rounded-3xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-black text-xl text-slate-900 dark:text-white">{editingBillId ? 'Edit Tagihan' : 'Tambah Tagihan'}</h3>
              <button onClick={() => { setShowBillModal(false); setEditingBillId(null); }} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center"><X size={16} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-widest">Nama Tagihan</label>
                <input type="text" placeholder="Netflix, Spotify, Listrik..." className="w-full bg-slate-50 dark:bg-[#111] px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none text-sm font-medium text-slate-900 dark:text-white" value={billForm.name} onChange={e => setBillForm({ ...billForm, name: e.target.value })} />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-widest">Nominal (Rp)</label>
                <input type="number" placeholder="150000" className="w-full bg-slate-50 dark:bg-[#111] px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none text-sm font-medium text-slate-900 dark:text-white" value={billForm.amount} onChange={e => setBillForm({ ...billForm, amount: e.target.value })} />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-widest">Tanggal Jatuh Tempo (1-31)</label>
                <input type="number" min={1} max={31} placeholder="15" className="w-full bg-slate-50 dark:bg-[#111] px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none text-sm font-medium text-slate-900 dark:text-white" value={billForm.due_date} onChange={e => setBillForm({ ...billForm, due_date: e.target.value })} />
              </div>
              <button onClick={handleSaveBill} disabled={isSavingBill} className="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-500/25">
                {isSavingBill ? <Loader2 className="animate-spin" size={18} /> : <>{editingBillId ? 'Perbarui Tagihan' : 'Simpan Tagihan'}</>}
              </button>
            </div>
          </div>
        </div>
